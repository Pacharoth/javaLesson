...
public class CallingCost {
    ...
    public boolean checkTime(int minutesch,int secondsch){
        if(minutesch>59){
            return false;
        }
        if(secondsch>59){
    ...
    public void showResult(){
        ...
        if(checkTime(startminutes, startseconds)&&checkTime(endminutes, endseconds)){
            if(resultend>resultstart){
                ...
                System.out.println("\nTime end cant be smaller than start time");
            }
        }else{
            System.out.println("\nInvalid Time");
        }
